...
#set page(
  ...
  footer: context {
    ...
    grid(
      ...
      [Analytics · self-hosted{{ if $.BaseURL }} · {{ typstMD $.BaseURL }}{{ end }} · {{ typstMD $.Property.Name }} · {{ typstMD $.Dash.DateStart }} → {{ typstMD $.Dash.DateEnd }}],
...
#block(
  ...
  #grid(
    ...
    [
      ...
      #text(font: mono, size: 8.5pt)[{{ typstMD $.Property.ID }}]
    ],
    [
    ...
    [
      ...
      #text(weight: "semibold")[{{ typstMD $.Dash.DateStart }} → {{ typstMD $.Dash.DateEnd }}] #text(fill: dim)[ ({{ $.Dash.DateRange }} days)]
    ],
    [
      #text(size: 7.5pt, tracking: 0.4pt, fill: dim, upper("Live users · last 30m")) \
      #text(weight: "semibold")[{{ $.Dash.LiveUsers }}]
    ],
  )
]
...
#box(stroke: 0.6pt + black, inset: (x: 6pt, y: 2pt))[
  #text(size: 8pt)[*Filter · url:* #text(font: mono)[{{ typstMD $.Dash.FilterURL }}]]
]
{{ end }}
...
#grid(
  ...
  gutter: 4pt,
  {{ range $.Dash.EventCards }}
  rect(width: 100%, stroke: 0.6pt + black, inset: 6pt)[
    #text(size: 7.5pt, tracking: 0.4pt, fill: dim)[{{ typstMD .Name }}]
    #v(2pt)
    #text(size: 14pt, weight: "bold")[{{ typstMD .Value }}]
    #v(1pt)
    {{ if eq .PercentChange 0 }}
    #text(size: 7.5pt, fill: rgb("#333"))[· no change]
    {{ else if lt .PercentChange 0 }}
    #text(size: 7.5pt, fill: rgb("#333"))[▼ {{ .PercentChange }}% vs previous]
    {{ else }}
    #text(size: 7.5pt, fill: rgb("#333"))[▲ +{{ .PercentChange }}% vs previous]
    {{ end }}
  ],
...
#grid(
  ...
  column-gutter: 10pt,
  row-gutter: 8pt,
  {{ if $.Dash.PageViewsByPageURL }}
  [
    === Top pages · page views
    #label_count_table(
      label_header: "URL", count_header: "Views",
      (
        {{ range $.Dash.PageViewsByPageURL }}
        text(font: mono, size: 7pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}],
        {{ end }}
      ),
    )
  ],
  ...
  {{ end }}
  {{ if $.Dash.SessionsByReferrer }}
  [
    === Top referrers
    #label_count_table(
      label_header: "Referrer", count_header: "Sessions",
      (
        {{ range $.Dash.SessionsByReferrer }}
        text(font: mono, size: 7pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}],
        {{ end }}
      ),
    )
  ...
  {{ end }}
  {{ if $.Dash.ByCustomEvent }}
  [
    === Top custom events
    #label_count_table(
      label_header: "Event", count_header: "Count",
      (
        {{ range $.Dash.ByCustomEvent }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}],
        {{ end }}
      ),
    )
  ],
...
#grid(
  ...
  [
    ...
    #breakdown_table(
      ...
      (
        {{ range $.Dash.ByDevice }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}], text(size: 7.5pt, fill: dim)[{{ pct .Count $.Dash.BreakdownTotals.Device }}%],
        {{ end }}
      ),
    )
  ],
  ...
  [
    ...
    #breakdown_table(
      ...
      (
        {{ range $.Dash.ByBrowser }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}], text(size: 7.5pt, fill: dim)[{{ pct .Count $.Dash.BreakdownTotals.Browser }}%],
        {{ end }}
      ),
    )
  ],
  ...
  [
    ...
    #breakdown_table(
      label_header: "Name", count_header: "Events",
      (
        {{ range $.Dash.ByPlatform }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}], text(size: 7.5pt, fill: dim)[{{ pct .Count $.Dash.BreakdownTotals.Platform }}%],
        {{ end }}
      ),
    )
  ],
  {{ end }}
  {{ if $.Dash.ByScreenSize }}
  [
    === Screen size
    #breakdown_table(
      label_header: "Size", count_header: "Events",
      (
        {{ range $.Dash.ByScreenSize }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}], text(size: 7.5pt, fill: dim)[{{ pct .Count $.Dash.BreakdownTotals.ScreenSize }}%],
        {{ end }}
      ),
    )
  ],
...
{{ if or $.Dash.PageViewsByUTMSource $.Dash.PageViewsByUTMMedium $.Dash.PageViewsByUTMCampaig }}
== UTM attribution

#grid(
  columns: (1fr, 1fr, 1fr),
  column-gutter: 10pt,
  {{ if $.Dash.PageViewsByUTMSource }}
  [
    === Source
    #label_count_table(
      label_header: "Source", count_header: "Views",
      (
        {{ range $.Dash.PageViewsByUTMSource }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}],
        {{ end }}
      ),
    )
  ...
  {{ end }}
  {{ if $.Dash.PageViewsByUTMMedium }}
  [
    === Medium
    #label_count_table(
      label_header: "Medium", count_header: "Views",
      (
        {{ range $.Dash.PageViewsByUTMMedium }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}],
        {{ end }}
      ),
    )
  ...
  {{ end }}
  {{ if $.Dash.PageViewsByUTMCampaig }}
  [
    === Campaign
    #label_count_table(
      label_header: "Campaign", count_header: "Views",
      (
        {{ range $.Dash.PageViewsByUTMCampaig }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}],
        {{ end }}
      ),
    )
...
#grid(
  ...
  [
    ...
    #label_count_table(
      ...
      (
        text(size: 7.5pt, weight: "bold")[Total], text(size: 7.5pt, weight: "bold")[{{ $.Dash.Bots.Total }}],
        {{ range $.Dash.Bots.TopBots }}
        text(size: 7.5pt)[{{ typstMD .Label }}], text(size: 7.5pt)[{{ .Count }}],
        {{ end }}
      ),
    )
