...
// "at build time". That answer does not transfer here, and it is worth being
// explicit about why rather than looking like an oversight: a blog post's PDF
// is fully determined before the process boots, because posts are files that
// cannot change while it runs. A property report covers an arbitrary date
// range chosen in a query string over a table that grows every second. There
// is no finite set to precompile.
//
// So this is the one place in the repo where a subprocess sits on the request
// path, and the one runtime image that is not FROM scratch: it needs the typst
// binary and the fonts the template asks for.
