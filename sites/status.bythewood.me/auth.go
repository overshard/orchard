...
// version also accepted STATUS_COOKIE_SECRET, documented as optional, which is
// exactly the config-or-credential gray zone decisions/0008 set out to delete.
// Deriving has a real property too. Rotating the password invalidates every
// outstanding session, which is what someone rotating a password wants and
// what a separate secret would quietly fail to do.
...
//
// The payload is "1:<unix expiry>", the Rust format. tower-cookies signed the
// whole cookie with its own scheme; this signs the payload with HMAC-SHA256
// and appends it, which is the same guarantee stated in code the reader can
// see. Sessions do not survive the cutover either way, since the key differs.
//
...
//
// A bare "starts with /" test is not enough, and the Rust version had exactly
// that with one exclusion: "//evil.example" also starts with a slash and is a
// protocol-relative URL that browsers follow off-site, which it did exclude,
// but "/\evil.example" is treated the same way by some browsers and it did
// not.
...
// arrives as a 200 full of HTML that fails to parse as JSON, which is how a
// clear "you are logged out" becomes a console error about an unexpected "<".
