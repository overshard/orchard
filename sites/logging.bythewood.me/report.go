...
// Report export: the same dashboard data, rendered as Typst markup and compiled
// to PDF, or as Markdown and served directly. Whatever window and source the
// page was showing is what the report covers, so a report is a snapshot of the
// screen rather than a second definition of the same numbers.
...
var reportFuncs = texttemplate.FuncMap{
	...
	// The same two formatters the HTML pages use, so a number reads
	// identically on screen and in the PDF. A report that rounded
	// differently from the dashboard it was exported from would be a
	// discrepancy somebody eventually tries to explain.
	"ms":  formatMS,
	"num": formatNum,
	"md":  markdownCell,
}

// markdownCell escapes a value for a Markdown table cell.
//
// It exists because report.typ routes every log field through typstMD and
// report.md, for a while, routed none of them through anything. That mattered
// more than it looks: `path` is publicly attacker-controlled, because
// web.Logged records r.URL.Path and Go percent-decodes it, so a request to
// /x%0A%7C... stores a path containing a real newline and a pipe. Rendering a
// report over it produced fabricated table rows carrying invented numbers, plus
// surviving raw HTML and a javascript: link. Anybody able to make a request to
// any of these sites could poison the operator's own report.
//
// Newlines become spaces because a cell cannot contain one: that alone stops
// the row forgery. The rest is the set that has meaning in a cell, plus the
// HTML-significant characters, since Markdown passes raw HTML straight through
...
// asciiFilename reduces a report subject to something a Content-Disposition
