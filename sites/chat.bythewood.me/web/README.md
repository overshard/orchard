...
Chat's own copy of the shared pieces, the same way every other site in orchard
carries one. The renderer, static handler and Vite manifest are still absent,
because chat parses its templates itself and serves assets out of `assets_embed.go` and `assets_disk.go`.
