...
and Caddy that front them. Ten sites, all Go, all served from a desktop behind a
...
targets and an editor can see all ten at once, and nothing depends on it. Each
...
**A fix in `web/` has to be made ten times.** Do not add a shared parent module
to avoid it, and keep `shipper.go` and `session.go` byte identical across the
ten, since both are wire formats as much as files.
...
`orchard-auth`, `orchard-chat`, `orchard-llm`
...
`web/shipper_test.go` is one of the twelve identical copies and covers the parts
